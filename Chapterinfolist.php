...
<?php
	 ...
	 $colname_ChapterInfo="-1";
	 if(isset($_POST['chapterName'])){
		 $colname_ChapterInfo=$_POST['chapterName'];
	 }    
	 //sprintf函数生成字符串
	 $query_ChapterInfo=sprintf("select * from chapter where chaptername='%s'",$colname_ChapterInfo);
	 mysqli_query($conn,"set names 'utf8'");//设置字符编码格式
	 $chapterinfo=mysqli_query($conn,$query_ChapterInfo) or die(mysqli_error($conn));//查询数据
	 //mysqli_fetch_assoc从结果集中取出一行数据，成为相关数组，键名是字段名，值是数据
	 $row_courseinfo=mysqli_fetch_assoc($chapterinfo);//
?>
...
<head>
	...
	<table border="0" cellspacing="" cellpadding="" width="100%">
		...
		<td><a href="deletepaper.php">删除章节</a></td>
            </tr>
            <tr><td colspan="6" >
		<table border="0"  width="100%">
                    <tr><td colspan="6" align="center"><h2>所有章节</h2></td></tr>
                    <tr><td  valign="middle" align="center"><b>本章所属的课程id</b></td>
                        ...
                        <tr valign="middle" align="center">
                            ...
                            <td><?php echo $row_courseinfo['gmt_create'];?></td>
                            <td><?php echo $row_courseinfo['gmt_modified'];?></td>
                        </tr>
                        <?php }while ($row_chapterinfo= mysqli_fetch_assoc($chapterinfo));
                            ?>
		</table>
                </td>
            </tr>  
		    ...
		    <hr>
		    <tr width="100%"><td align="center" valign="middle">Copyright@2020 lanmo</td></tr>
