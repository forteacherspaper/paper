...
-- 正在导出表  paper.paper 的数据：~3 rows (大约)
...
INSERT INTO `question` (`id`, `sectionid`, `question`, `questiontypeid`, `answer`, `username`, `isdeleted`, `gmt_create`, `gmt_modified`) VALUES
	...
	(4, 4, 'What is the typical scene at Memphis Airport every night? (Para.1)', '简答题', 'Then they are loaded onto other aircraft, and flown to their destinations – many within 24 hours of 

leaving their senders.', '李艺彤', 0, '2020-07-10 16:30:48', '2020-07-10 16:30:50'),
	...
	(14, 4, 'Can you find the topic sentence in Paragraph 2? ', '简答题', 'Smith is recognized as an outstanding entrepreneur with an agreeable and winning personality.', '李艺彤', 0, '2020-07

-10 16:54:06', '2020-07-10 16:54:07'),
...
\nC、 0.60\r\nD、 不确定', '单选题', 'A', 'Wt', 0, NULL, NULL),
	...
	(40, 17, 'C语言中,基本整型类型关键字为__________,在16位操作系统中,一般用__________字节表示一个变量。单精度浮点型型类型关键字为__________,一般占__________字节。', '填空题', 'int；2；float；4
...
用switch的分支,都可以通过if实现', '单选题', 'C', 'Wt', 0, NULL, NULL),
...
入:\r\n5\r\n输出:', '填空题', '120', 'Wt', 0, NULL, NULL),
	(90, 39, '以下能对一维数组a进行初始化的语句是(  )。\r\nA、 int a[5]=(0,1,2,3,4) \r\nB、  int a(5)={ } \r\nC、  int a[3]={0,1,2} \r\nD、 int a{5}={10*1}', '单选题', 'C', 'Wt', 0, NULL, 
...
数返回一指向int型数据的指针', '单选题', 'C', 'Wt', 0, NULL, NULL),
...
语句将一组常量赋给一个结构体变量', '单选题', 'B', 'Wt', 0, NULL, NULL),
	(131, 57, '结构体类型变量在程序执行期间，（  ）。\r\n \r\nA、 所有成员一直驻留在内存中。\r\n \r\nB、 只有一个成员驻留在内存中。\r\n \r\nC、 部分成员驻留在内存中。\r\n \r\nD、 没有成员驻留在

内存中。', '单选题', 'A', 'Wt', 0, NULL, NULL),
	(132, 57, '设有以下语句\r\nstruct stu\r\n{\r\n   int a;\r\n   float b;\r\n}stutype;\r\n则下面不正确的叙述是（  ）。\r\n \r\nA、 struct是结构体类型的关键字\r\n \r\nB、 struct stu是用户自定义
...
==NULL)\r\n  { printf("不能打开文件");\r\n   getch();\r\n   exit（1）;\r\n }                \r\n  fclose(fp);\r\n}', '填空题', 'fprintf(fp,"%c",i);', 'Wt', 0, NULL, NULL),
	...
	(162, 36, '建立类 cylinder, cylinder 的构造函数被传递了两个 double 值，分别表示圆柱体的半径和高度。用类 cylinder 计算圆柱体的体积，并存储在一个 double 变量中。在类 cylinder 中包含一个成员函

数 vol，用来显示每个 cylinder 对象的体积', '应用题', '', 'Wt', 0, NULL, NULL),
	...
	(168, 59, '构建一个类book，其中包含两个私有数据成员qu和price，将qu初始化为1~5，将price初始化为qu的10倍，建立一个有5个元素的数组对象。显示每个数组元素的qu*price的值。', '应用题', '', 'Wt', 

0, NULL, NULL),
	(169, 29, '编写一个程序，已有若干学生的数据，包括学号、姓名、成绩，要求输出这些学生的数据并计算出学生人数和平均成绩（要求将学生人数和总成绩用静态数据成员表示）', '应用题', '', 'Wt', 0, 

NULL, NULL),
	(170, 34, '编写函数void reverse（string &s），用递归算法使字符串S倒序。提示： substr有2种用法： 假设：string s = “0123456789”;   string sub1 = s.substr(5); //只有一个数字5表示从下标为5开

始一直到结尾：sub1 = “56789”   string sub2 = s.substr(5, 3); //从下标为5开始截取长度为3位：sub2 = “567”  ', '应用题', '', 'Wt', 0, NULL, NULL);
